...
static ASTNode* makeSymbol(QChar ch){ auto n=new ASTNode(); n->type=ASTNode::Symbol; n->value=QString(ch); return n; }
static ASTNode* makeConcat(ASTNode* a, ASTNode* b){ auto n=new ASTNode(); n->type=ASTNode::Concat; n->children={a,b}; return n; }
static ASTNode* makeCharSet(const QString& chars){ auto n=new ASTNode(); n->type=ASTNode::CharSet; n->value=chars; return n; }
static ASTNode* makeKeywordCI(const QString& word){ if(word.isEmpty()) return nullptr; auto first = makeCharSet(QString(word[0]).toLower()+QString(word[0]).toUpper()); ASTNode* seq = first; for(int i=1;i<word.size();++i){ QString ch = QString(word[i]); auto cs = makeCharSet(ch.toLower()+ch.toUpper()); seq = makeConcat(seq, cs); } return seq; }

QVector<MinDFA> Engine::buildAllMinDFA(const ParsedFile& pf, QVector<int>& codes){ QVector<MinDFA> result; codes.clear(); for(const auto& pt : pf.tokens){ if(pt.rule.isGroup){ QVector<ASTNode*> alts; collectAlternatives(pt.ast, alts); int base = pt.rule.code; int idx=0; for(auto alt : alts){ auto nfa = buildNFA(alt, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(base + (idx++)); }
            if(alts.size()<=1 && pt.rule.name.startsWith("_keywords", Qt::CaseInsensitive)){
                QStringList kws({"if","then","else","end","repeat","until","read","write"});
                for(auto kw : kws){ auto ast = makeKeywordCI(kw); auto nfa = buildNFA(ast, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(base + (idx++)); }
            }
        } else { auto nfa = buildNFA(pt.ast, pf.alpha); auto dfa = buildDFA(nfa); auto mdfa = buildMinDFA(dfa); result.push_back(mdfa); codes.push_back(pt.rule.code); } } return result; }
