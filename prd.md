...
5.  **测试与验证区 (Testing Panel)**：
    ...
    *   **左侧/上方**：**源程序输入框**（加载示例源文件内容，如 `sample.tny`）。
    *   **操作按钮**：[词法分析/Run Lexer]。
...
2.  使用外部 C/C++ 编译器（如 GCC, MSVC）编译该源代码，生成可执行文件（例如 `lexer.exe`）。

### 第三步：运行测试
1.  准备一个源代码文件（如 TINY 的 `sample.tny`，也可为其他语言的源码）。
2.  运行生成的 `lexer.exe`，读取上述源文件。
