...
## 关键代码（可选）
```
// FIRST/FOLLOW 计算（项目源码节选，不超过100行）
static QSet<QString> firstSeq(const Grammar& g, const QVector<QString>& seq, const QMap<QString, QSet<QString>>& first)
{
    QSet<QString> r;
    bool          allEps = true;
    for (const auto& s : seq)
    {
        if (g.terminals.contains(s)) { r.insert(s); allEps = false; break; }
        auto fs = first.value(s);
        for (const auto& t : fs) if (t != Config::epsilonSymbol()) r.insert(t);
        if (!fs.contains(Config::epsilonSymbol())) { allEps = false; break; }
    }
    if (allEps) r.insert(Config::epsilonSymbol());
    return r;
}
...
LL1Info LL1::compute(const Grammar& g)
{
    ...
    for (const auto& a : g.terminals) { info.first.insert(a, QSet<QString>()); info.first[a].insert(a); }
    bool changed = true;
    while (changed)
    {
        changed = false;
    ...
    if (!g.startSymbol.isEmpty()) info.follow[g.startSymbol].insert(Config::eofSymbol());
    changed = true;
    while (changed)
    {
        changed = false;
        for (auto it = g.productions.begin(); it != g.productions.end(); ++it)
        {
            ...
            for (const auto& p : it.value())
            {
                for (int i = 0; i < p.right.size(); ++i)
                {
                    ...
                    QVector<QString> beta; for (int j = i + 1; j < p.right.size(); ++j) beta.push_back(p.right[j]);
                    auto fbeta = firstSeq(g, beta, info.first);
                    int  prev  = info.follow[B].size();
                    for (const auto& x : fbeta) if (x != "#") info.follow[B].insert(x);
                    if (beta.isEmpty() || fbeta.contains("#")) { for (const auto& x : info.follow[A]) info.follow[B].insert(x); }
