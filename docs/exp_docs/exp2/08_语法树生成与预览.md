...
对象或变量名称 | 数据结构 | 存储结构用途
| - | - | -
ParseTreeNode | 结构体 | 语法树节点的符号(QString)与孩子列表(QVector<ParseTreeNode*>)
SemanticASTNode | 结构体 | 语义树节点的标签(QString)与孩子列表(QVector<SemanticASTNode*>)
identifierTokenNames | QVector<QString> | 标识符类终结符名称集合（词素增强）
roles/roleMeaning | QMap<int, QString>/QVector<int> | 候选位角色定义与含义（root/child/sibling）
rootPolicy/childOrder | QString | 根选择与孩子附加顺序策略
nodeStk/semStk | QVector<ParseTreeNode*>/QVector<SemanticASTNode*> | 语法树/语义树构建过程的节点栈
lexemes | QVector<QString> | 词素序列（与 Token 对齐，可选）
...
3. 语义构建：对于归约，按 `roles/roleMeaning` 将某些孩子标记为根/孩子/并列兄弟；
...
### 扩展要点（细节说明）
- 标识符类处理：当终结符名属于标识符集合时，语义树可将词素节点直接作为根或孩子以增强可读性；否则按普通终结符处理。
- 角色冲突：当同一归约候选存在多个 root 标注时，构造聚合根以并列附加，避免丢失任一关键子树。
- 子序策略：`rhs_order` 按右部顺序附加；反序策略则自右向左附加，有助于某些语义树的惯常表示（如表达式树）。
- DOT 生成：生成节点标签与边关系时保留语义标签与层次；对特殊字符进行转义，确保渲染兼容。
